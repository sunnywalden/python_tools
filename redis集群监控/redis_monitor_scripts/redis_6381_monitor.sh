...
dir_name=$(dirname $0)
cnf_file=$dir_name/redis_conf.sh
test -s $cnf_file && . $cnf_file
#cmd_prefix="/opt/.venv/bin/python $dir_name/redis_info.py --host $host --port $port --pswd $passwd"
...
log_file=/var/log/monitor_redis_6381.log
...
monitor_redis() {
    case $1 in
        connected_clients|used_memory|role)
            ...
            /usr/local/zabbix/bin/zabbix_sender -z $zabbix_server -s $host -k redis_6381.$1 -o $val
            ;;
        *)
