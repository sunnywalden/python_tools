...
class MongoDB(object):
    ...
    def send_metrics(self):
        """send all metrics to zabbix server"""
        metrics = self.__metrics
        for metric in metrics:
            zabbix_item_key = str(metric['key'])
            zabbix_item_value = str(metric['value'])
            #print('- ' + zabbix_item_key + ' ' + zabbix_item_value)
            zabbix_helper.send_to_zabbix_server(zabbix_item_key, zabbix_item_value)
    ...
    def get_db_names(self):
        ...
        dict_metrics = {}
        dict_metrics['key'] = 'mongodb.'+ str(self.mongo_port) +'.ismaster'
        if master:
    ...
    def get_mongo_db_lld(self):
        ...
        dict_metrics = {}
        db_list = []
        dict_metrics['key'] = 'mongodb.'+ str(self.mongo_port) +'.discovery'
    ...
    def get_oplog(self):
        ...
        if op_first.count() > 0 and op_last.count() > 0:
            ...
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.oplog', status)

            current_time = timegm(gmtime())
            oplog = int(((str(op_last_st).split('('))[1].split(','))[0])
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.oplog-sync', (current_time - oplog))
    ...
    def get_maintenance(self):
        ...
        self.add_metrics('mongodb.'+ str(self.mongo_port) +'.fsync-locked', fsync_locked)
        ...
        try:
            ...
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.priority', priority)
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.hidden', hidden)
    ...
    def get_server_status_metrics(self):
        ...
        # db info
        self.add_metrics('mongodb.'+ str(self.mongo_port) +'.version', ss['version'].replace('.', '', 3))
        self.add_metrics('mongodb.'+ str(self.mongo_port) +'.storageEngine', ss['storageEngine']['name'])
        self.add_metrics('mongodb.'+ str(self.mongo_port) +'.uptime', int(ss['uptime']))
        self.add_metrics('mongodb.'+ str(self.mongo_port) +'.okstatus', int(ss['ok']))

        # asserts
        for k, v in ss['asserts'].items():
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.asserts.' + k, v)

        # operations
        for k, v in ss['opcounters'].items():
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.operation.' + k, v)

        # memory
        for k in ['resident', 'virtual', 'mapped', 'mappedWithJournal']:
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.memory.' + k, ss['mem'][k])

        # connections
        for k, v in ss['connections'].items():
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.connection.' + k, v)

        # network
        for k, v in ss['network'].items():
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.network.' + k, v)

        # extra info
        self.add_metrics('mongodb.'+ str(self.mongo_port) +'.page.faults',
                         ss['extra_info']['page_faults'])

        #wired tiger
        if ss['storageEngine']['name'] == 'wiredTiger':
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.used-cache',
                             ss['wiredTiger']['cache']
                             ["bytes currently in the cache"])
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.total-cache',
                             ss['wiredTiger']['cache']
                             ["maximum bytes configured"])
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.dirty-cache',
                             ss['wiredTiger']['cache']
                             ["tracked dirty bytes in the cache"])

        # global lock
        lock_total_time = ss['globalLock']['totalTime']
        self.add_metrics('mongodb.'+ str(self.mongo_port) +'.globalLock.totalTime', lock_total_time)
        for k, v in ss['globalLock']['currentQueue'].items():
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.globalLock.currentQueue.' + k, v)
        for k, v in ss['globalLock']['activeClients'].items():
            self.add_metrics('mongodb.'+ str(self.mongo_port) +'.globalLock.activeClients.' + k, v)
            ...
            for mongo_db in self.__dbnames:
                ...
                for k, v in dbs.items():
                    if k in ['storageSize', 'ok', 'avgObjSize', 'indexes',
                             ...
                             'nsSizeMB']:
                        self.add_metrics('mongodb.'+ str(self.mongo_port) +'.stats.' + k +
                                         '[' + mongo_db + ']', int(v))
    def close(self):
...
def sent_metrics(mongodb):
    ...
    mongodb.send_metrics()
    mongodb.close()

if __name__ == '__main__':
    mongodb_27002 = MongoDB('10.150.28.50', 27002)
    mongodb_27003 = MongoDB('10.150.28.50', 27003)
    mongodb_27004 = MongoDB('10.150.28.50', 27004)
